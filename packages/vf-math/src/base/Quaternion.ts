...
class Quaternion extends AbstractMathObject<QuaternionLike> {
	...
	static slerpFlat(dst: number[], dstOffset: number, src0: number[], srcOffset0: number, src1: number[], srcOffset1: number, t: number): void {
		...
		if (w0 !== w1 || x0 !== x1 || y0 !== y1 || z0 !== z1) {
			...
			if (sqrSin > Tolerance.CALCULATION_EPS) {
	...
	slerp(qb: Quaternion, t: number): this {
		...
		if (sqrSinHalfTheta <= Tolerance.CALCULATION_EPS) {
			const s = 1 - t;
	...
	equals(quaternion: Quaternion, eps = Tolerance.LENGTH_EPS): boolean {
		return MathUtils.equals(this._x, quaternion._x, eps) &&
			MathUtils.equals(this._y, quaternion._y, eps) &&
			MathUtils.equals(this._z, quaternion._z, eps) &&
			MathUtils.equals(this._w, quaternion._w, eps);
	}
